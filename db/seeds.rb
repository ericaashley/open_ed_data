...
school_title_ones.each do |nces_id, title_one|
  SchoolTitleOne.find_or_create_by!(nces_id: nces_id,
...
CSV.foreach('db/seeds/sc121a_supp.txt', col_sep: "\t", headers: true) do |row|
  ...
  district = District.find_or_initialize_by(nces_id: row["LEAID"],
    district_name: row["LEANM"], state_id: row["FIPST"])
  district.save!
